...
Plugin URI: 
Description: A simple popup that ajax loads content from another WP page/post - on window load or click. Shortcode: [tha_popup_link page_id="123" lazy_load="true|onclick" link_text="Click Me" bg="#fff" color="#000" classes=""]
Version: 1.0
...
function tha_popup_content($pgid, $title=''){
   $page_data = get_post($pgid);
   if($page_data){
      if($title){
        $title = '<h2>' . get_the_title($pgid) . '</h2>';
       }
       $content = $title;
...
function tha_popup_ajax(){
  if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
    ...
    if($id){
      $data['id'] = $id;
      $data['content'] =  tha_popup_content($id, $title); 
